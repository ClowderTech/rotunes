...
export async function execute(interaction: CommandInteraction) {
    ...
    if (!player.playing) {
        throw new Error("Goofy ahh library breaks every time. Now, we wait for the next update.");
    }

    let volume = <number>interaction.options.get("volume", true).value;
    ...
    if (!(member.roles.cache.some(role => role.name === "DJ") || member.permissions.has("ModerateMembers", true) || member.voice.channel.members.filter(member => !member.user.bot).size <= 2)) {
        let embed = new EmbedBuilder()
            ...
            .setDescription(`You are not a DJ, so you need to vote to set the volume. React with ✅ to set it. You have 30 seconds to vote. Have ${Math.ceil(member.voice.channel.members.filter(member => !member.user.bot).size / 2)} votes to set the loudness.`)
            .setTimestamp()
            .setColor("#2b2d31")
